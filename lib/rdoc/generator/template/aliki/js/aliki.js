...
const headerToDepth = { "h1": 0, "h2": 1, "h3": 2, "section": 1 };
...
function generateToc() {
  ...
  const headings = main.querySelectorAll('h1, h2, h3, .documentation-section:not(.documentation-section-toplevel)');
  ...
  headings.forEach((heading) => {
    ...
    let level = headerToDepth[heading.tagName.toLowerCase()];
    if (heading.closest(".documentation-section:not(.documentation-section-toplevel)")) {
      level += 1;
    }
    const li = document.createElement('li');
    li.className = `toc-item toc-depth-${level}`;

    const link = document.createElement('a');
    link.href = `#${heading.id}`;
    link.className = 'toc-link';
    link.setAttribute('data-target', heading.id);
    if (heading.tagName.toLowerCase() === "section") {
      link.textContent = heading.querySelector(".documentation-section-title").textContent;
    } else {
