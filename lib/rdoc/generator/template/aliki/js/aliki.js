...
function hookBackToTop() {
  ...
  var main = document.querySelector('main');
  var baseInlineOffset = 16; // Matches var(--space-4)

  function toggleVisibility() {
    var isVisible = window.scrollY > triggerOffset;
  ...
  function updateInlineOffset() {
    ...
    var gap = Math.max(0, viewportWidth - rect.right);
    if (gap > baseInlineOffset) {
      button.style.right = (gap + baseInlineOffset) + 'px';
    } else {
      button.style.removeProperty('right');
    }
  }
  ...
  button.addEventListener('click', function() {
    ...
    if (!reduceMotion && supportsSmoothScroll) {
  ...
  window.addEventListener('resize', updateInlineOffset);
