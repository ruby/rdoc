...
function hookSearch() {
  ...
  document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape' && search.isShown()) {
      search.hide();
      input.blur();
    }
  });

  // Press 's' or '/' to focus search input
  document.addEventListener('keydown', (e) => {
    if (e.key === 's' || e.key === '/' && !search.isShown()) {
      e.preventDefault(); // Prevent the 's' '/' character from being typed
      input.focus();
    }
  });
