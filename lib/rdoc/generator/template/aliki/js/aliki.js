...
function hookSidebarFilter() {
  const filterInput = document.querySelector('#class-module-filter-input');
  const clearButton = document.querySelector('#class-module-filter-clear');
  const classList = document.querySelector('#sidebar-class-list');
  const noResults = document.querySelector('#class-module-filter-no-results');

  if (!filterInput || !classList) return;

  // Store original HTML for reset
  const originalHTML = classList.innerHTML;
  ...
  function filterClasses(query) {
    ...
    // Reset to original state if empty query
    if (!normalizedQuery) {
      classList.innerHTML = originalHTML;
    ...
    const allItems = classList.querySelectorAll('li');

    // First pass: hide all items and reset highlights
    allItems.forEach(li => {
    ...
    // Second pass: show items that match and their ancestors
    ...
    allItems.forEach(li => {
      ...
      if (matches) {
        ...
        highlightMatch(link, normalizedQuery);

        // Show all ancestor elements
    ...
    if (noResults) {
      noResults.hidden = hasVisibleItems;
    }
  }

  // Get the link element from a list item (handles <code> wrapper)
  function getLink(li) {
    return li.querySelector('.sidebar-class-link');
  }

  // Highlight matching text in link
  function highlightMatch(link, query) {
    ...
    link.innerHTML = `${escapeHTML(before)}<mark class="filter-highlight">${escapeHTML(match)}</mark>${escapeHTML(after)}`;
  }
  ...
  // Event listeners
  const debouncedFilter = debounce((e) => filterClasses(e.target.value), 150);
