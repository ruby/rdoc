...
function hookScrollSpy() {
  ...
  function setActiveAnchor() {
    ...
    for (let i = anchorsArray.length - 1; i >= 0; i--) {
      const anchor = anchorsArray[i];
      const rect = anchor.getBoundingClientRect();
      
      // Check if the anchor is in the viewport
      if (rect.top <= windowHeight / 2 && rect.bottom >= 0) {
        const id = anchor.id;
        document.querySelectorAll('.contextual li').forEach(li => {
    ...
    // If no anchor is active, keep the last one active
    if (!activeAnchorFound) {
      if (lastActiveLi) {
        lastActiveLi.classList.add('active');
      }
    }
  }
  ...
  setActiveAnchor(); // Call once to set initial state
}
